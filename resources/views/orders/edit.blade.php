<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            <a href="{{ route('panel.orders.index') }}">&laquo; back</a> / {{ __('Edit order') }} #{{ $order->id }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                <form method="POST" action="{{ route('panel.orders.update', ['order' => $order->id]) }}">
                    @csrf
                    @method('PATCH')
                    ...
                    <div>
                        ...
                        <x-text-input id="customer_fullname" class="block mt-1 w-full" type="text" name="customer_fullname" :value="$order->customer_fullname" required autofocus autocomplete="customer_fullname" />
                    ...
                    <div class="mt-4">
                        ...
                        <x-text-input id="customer_comment" class="block mt-1 w-full" type="text" name="customer_comment" :value="$order->customer_comment" />
                    ...
                    <div class="mt-4">
                        ...
                        <select id="status" name="status" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm">
                            @foreach ($statuses as $code => $name)
                                <option value="{{ $code }}" {{ $code == $order->status ? 'selected' : '' }}>
                                    {{ $name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mt-4">
                        ...
                        <x-text-input id="product_count" class="block mt-1 w-full" type="text" name="product_count" :value="$order->product_count" />
